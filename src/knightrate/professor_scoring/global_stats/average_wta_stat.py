...
class AverageWouldTakeAgainStat(StatisticStrategy):
    """Calculates the average would_take_again score across every professor."""

    def __init__(self):
        self.metric_name = "avgWouldTakeAgain"

    def calculate(self, professors: list[Professor]) -> float:
        valid_wta = [p.scores.would_take_again_score for p in professors if p.scores and 
        p.scores.would_take_again_score != "Unavailable"]
